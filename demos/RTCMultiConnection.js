...
window.RTCMultiConnection = function (roomid, forceOptions) {
    ...
    function SocketConnection(connection, connectCallback) {
        ...
        //返回用户信息
        connection.socket.on('return-userlist', function (responseCode, data) {
           

        });
        connection.socket.on('join-our-room', function (roomid) {
    ...
    (function (connection) {
        ...
        connection.notify = function (remoteUserId, localUserid) {
            if (connection.socket) {
                connection.socket.emit('notify-remoteId', remoteUserId, localUserid)
            }
        }
        connection.setLngLat = function (userId, lng, lat, type) {
            var data = {};
            data = {
                userId: userId,
                lng: lng,
        ...
        connection.login = function (localUserid, isTeamMember, isPublicModerator, callback) {
            ...
            connectSocket(function () {
                ...
                connection.socket.emit('changed-uuid', connection.userid, isTeamMember);
                //获取在线用户列表
                callback();
            });
        }

        connection.open = function (localUserid, remotename, isPublicModerator, callback) {
            connection.waitingForLocalMedia = true;
            connection.isInitiator = true;
            ...
            var demo1 = connection.notify(remotename, localUserid);
        };
        ...
        connection.join = connection.connect = function (remoteUserId, options) {
            ...
            if (typeof options.localPeerSdpConstraints !== 'undefined') {
                console.log("3、+sesionid======================================");
                localPeerSdpConstraints = options.localPeerSdpConstraints;
            }

            if (typeof options.remotePeerSdpConstraints !== 'undefined') {
                console.log("4、+sesionid======================================");
                remotePeerSdpConstraints = options.remotePeerSdpConstraints;
            }

            if (typeof options.isOneWay !== 'undefined') {
                console.log("5、+sesionid======================================");
            ...
            beforeJoin(connectionDescription.message, function () {
                console.log("7、+sesionid======================================");
                connectSocket(function () {
                    if (!!connection.peers[connection.sessionid]) {
                        // on socket disconnect & reconnect
                        console.log("8、+sesionid======================================");
                        return;
                    }
        ...
        function beforeJoin(userPreferences, callback) {
            ...
            if (userPreferences.localPeerSdpConstraints.OfferToReceiveAudio) {
                console.log("10、+sesionid======================================");
                localMediaConstraints.audio = connection.mediaConstraints.audio;
            }

            if (userPreferences.localPeerSdpConstraints.OfferToReceiveVideo) {
                console.log("11、+sesionid======================================");
            ...
            if (session.oneway && session.audio !== 'two-way' && session.video !== 'two-way' && session.screen !== 'two-way') {
                console.log("12、+sesionid======================================");
                callback();
                return;
            }

            if (session.oneway && session.audio && session.audio === 'two-way') {
                console.log("13、+sesionid======================================");
                session = {
                    audio: true
                };
            }
        ...
        connection.invokeGetUserMedia = function (localMediaConstraints, callback, session) {
            console.log("23、+invokeGetUserMedia======================================");
            if (!session) {
                session = connection.session;
            }

            if (!localMediaConstraints) {
                console.log("24、+invokeGetUserMedia======================================");
                localMediaConstraints = connection.mediaConstraints;
            }

            getUserMediaHandler({
                onGettingLocalMedia: function (stream) {
                    console.log("25、+invokeGetUserMedia======================================");
                    var videoConstraints = localMediaConstraints.video;
                    if (videoConstraints) {
                        console.log("26、+invokeGetUserMedia======================================");
                        if (videoConstraints.mediaSource || videoConstraints.mozMediaSource) {
                            console.log("27、+invokeGetUserMedia======================================");
                            stream.isScreen = true;
                        } else if (videoConstraints.mandatory && videoConstraints.mandatory.chromeMediaSource) {
                            console.log("28、+invokeGetUserMedia======================================");
                            stream.isScreen = true;
                        }
                    }

                    if (!stream.isScreen) {
                        console.log("29、+invokeGetUserMedia======================================");
                    ...
                    mPeer.onGettingLocalMedia(stream, function () {
                        console.log("30、+invokeGetUserMedia======================================");
