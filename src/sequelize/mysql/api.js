...
var Api = {
    ...
    //插入消息记录
    insertContent:(room_id,sender_id,content,userIds,type,chat_type,t)=>{
        var room_user_ids=''
        for(var i=0;i<userIds.length;i++)
        {
        ...
        return mysql.Content.create({ room_id,sender_id,content,type,chat_type,room_user_ids, sender_date: new Date().toString() }, { transaction: t })
    },
    //删除关系记录
        ...
        Api.isCommunicated(userIds).then((res) => {
            ...
            if (res.re==1) {
                ...
                mysql.sequelize.transaction().then((t) => {
                    ...
                    var second=userIds[1]
                    var roomInfo=null
                    Api.insertRoomInfo(`${first}_${second}`, t).then((room) => {
                        roomInfo=room
    ...
    //发送组消息和发送一对一消息都用此接口
    sendGroupMessage:(room_id,sender_id,content,userIds,type,chat_type)=>{
        var deferred = Q.defer()
        mysql.sequelize.transaction().then((t)=>{
            Api.insertContent(room_id,sender_id,content,userIds,type,chat_type,t).then((res)=>{
                deferred.resolve({re:1})
                t.commit()
            }).catch((e)=>{
                t.rollback()
